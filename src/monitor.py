...
def cbf(chip, gpio, level, tick):
    print("chip={} gpio={} level={} time={:.09f}".format(chip, gpio, level, tick / 1e9))
    ...
    if gpio >= 0:
        ...
        if handle >= 0:
            ...
            if err < 0:
                ...
                print("GPIO in use {}:{} ({})".format(chip, gpio, sbc.error_text(err)))
                exit()
            ...
            print("can't open gpiochip {} ({})".format(chip, sbc.error_text(handle)))
            exit()
